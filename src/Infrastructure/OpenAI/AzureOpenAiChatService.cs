...
public class AzureOpenAiChatService : IChatService
{
    ...
    public AzureOpenAiChatService(HttpClient httpClient, IConfiguration configuration)
    ...
    {
        //_logger.LogInformation("Preparing chat completion request for deployment {DeploymentId}", deploymentId);
        
        var endpoint = $"{_endpoint.TrimEnd('/')}/openai/deployments/{deploymentId}/chat/completions?api-version={_apiVersion}";
